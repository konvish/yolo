...
if training:
    while epoch < num_epochs + 1:
        ...
        loss_, _ = sess.run([PGNet.loss, PGNet.train_opt],
                            feed_dict={PGNet.inputs_: states_mb.reshape((len(states_mb), 100, 160, 4)),
                                       PGNet.actions: actions_mb,
                                       PGNet.discounted_episode_rewards_: discounted_rewards_mb})
        print("Training Loss: {}".format(loss_))
        summary = sess.run(write_op, feed_dict={
            PGNet.inputs_: states_mb.reshape((len(states_mb), 100, 160, 4)),
            PGNet.actions: actions_mb,
            PGNet.discounted_episode_rewards_: discounted_rewards_mb,
        ...
        if epoch % 10 == 0:
            saver.save(sess, "./models/doom_deathmatch.ckpt")
            print("Model saved")
        epoch += 1
...
with tf.Session() as sess:
    ...
    game.load_config("defend_the_center.cfg")
    game.set_doom_scenario_path("defend_the_center.wad")
    saver.restore(sess, "./models/doom_deathmatch.ckpt")
    game.init()
